...
def validate_dining_suggestion(cuisine, number_of_people, date, time, location, email):
    print("HERE----------------------------",cuisine, number_of_people, date, time, location, email)
...
def handle_dining_suggestion_intent(event):
    ...
    email = slots["Email"]
    # phone = slots["Phone"]
    print('jkashdbfsjkf',invocation_source)
    ...
    queue = sqs.get_queue_by_name(QueueName='MyQ')
    response = queue.send_message(MessageBody=json.dumps(queue_message))
    print('LF1 q response',response)
